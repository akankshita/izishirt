...
buildmenu:function(menuid, arrowsvar){
	jQuery(document).ready(function($){
		...
		$headers.each(function(i){
			...
			$curobj.children("a:eq(0)").css(this.istopheader? {paddingRight: "25px"} : {}).append('')
...
var arrowimg={down:['downarrowclass', 'down.gif', 13], right:['rightarrowclass', 'right.gif']}

var jquerytopmenu={

animateduration: {over: 200, out: 200}, //duration of slide in/ out animation, in milliseconds

buildmenutop:function(menuid, arrowsvar){
	jQuery(document).ready(function($){
		var $mainmenu=$("#"+menuid+">ul")
		var $headers=$mainmenu.find("ul").parent()
		$headers.each(function(i){
			...
			$curobj.hover(
				function(e){
					...
					menuleft=(this._offsets.left+menuleft+this._dimensions.subulw>$(window).width())? (this.istopheader? -this._dimensions.subulw+this._dimensions.w : -this._dimensions.w) : menuleft
					if ($targetul.queue().length<=1) //if 1 or less queued animations
						$targetul.css({left:menuleft+"px", width:this._dimensions.subulw+'px'}).slideDown(jquerytopmenu.animateduration.over)
				},
				function(e){
					var $targetul=$(this).children("ul:eq(0)")
					$targetul.slideUp(jquerytopmenu.animateduration.out)
				}
			) //end hover
		}) //end $headers.each()
		$mainmenu.find("ul").css({display:'none', visibility:'visible'})
...
//build menu with ID="myslidemenu" on page:
jquerytopmenu.buildmenutop("jquerytopmenu", arrowimg)
 ...
 * 
 * File generated: Wed May 19 08:25:32 GMT 2010
 */
