require 'uri'

class String
  # The extended characters map used by removeaccents. The accented characters
  # are coded here using their numerical equivalent to sidestep encoding issues.
  # These correspond to ISO-8859-1 encoding.
  ACCENTS_MAPPING = {
    'E' => [200,201,202,203],
    'e' => [232,233,234,235],
    'A' => [192,193,194,195,196,197],
    'a' => [224,225,226,227,228,229,230],
    'C' => [199],
    'c' => [231],
    'O' => [210,211,212,213,214,216],
    'o' => [242,243,244,245,246,248],
    'I' => [204,205,206,207],
    'i' => [236,237,238,239],
    'U' => [217,218,219,220],
    'u' => [249,250,251,252],
    'N' => [209],
    'n' => [241],
    'Y' => [221],
    'y' => [253,255],
    'AE' => [306],
    'ae' => [346],
    'OE' => [188],
    'oe' => [189]
  }


  # Remove the accents from the string. Uses String::ACCENTS_MAPPING as the source map.
  def removeaccents
    str = String.new(self)
    String::ACCENTS_MAPPING.each {|letter,accents|
      packed = accents.pack('U*')
      rxp = Regexp.new("[#{packed}]", nil, 'U')
      str.gsub!(rxp, letter)
    }

    str
  end


  # Convert a string to a format suitable for a URL without ever using escaped characters.
  # It calls strip, removeaccents, downcase (optional) then removes the spaces (optional)
  # and finally removes any characters matching the default regexp (/[^-_A-Za-z0-9]/).
  #
  # Options
  #
  # * :downcase => call downcase on the string (defaults to true)
  # * :convert_spaces => Convert space to underscore (defaults to false)
  # * :regexp => The regexp matching characters that will be converting to an empty string (defaults to /[^-_A-Za-z0-9]/)
  def urlize(options = {})
    options[:downcase] ||= true
    options[:convert_spaces] ||= false
    options[:regexp] ||= /[^-_A-Za-z0-9]/

    str = self.strip.removeaccents
    str.downcase! if options[:downcase]
    str.gsub!(/\ /,'_') if options[:convert_spaces]
    str.gsub(options[:regexp], '')
  end
end

class StringUtil < ActiveRecord::Base
  def self.pretty_escape(str)
    return URI.escape(str, Regexp.new("[^#{URI::PATTERN::UNRESERVED}]"))
  end

	def self.to_comparable_string(str)
		s = str.removeaccents	
		s = s.gsub("-", "")
		s = s.gsub(" ", "").strip
		s = s.gsub("	", "")
		s = s.gsub("\n", "")

		return s.downcase
	end

	def self.check_for_http(url)
		if ! url.index("http")
			return "http://#{url}"
		end

		return url
	end

	def self.limit_tag(tag, limit = 50)

		if tag.length <= limit
			return tag
		end

		new_tag = ""

		words = tag.split(" ")

		words.each do |word|
			tmp_word = word[0, limit - 1]

			if "#{new_tag}#{tmp_word} ".length > limit
				break
			end
			
			new_tag = new_tag + "#{tmp_word} "		
		end

		return new_tag
	end

  def self.create_callname(string)

    string = string.removeaccents

    callname = string.dup
    callname.gsub!(/[Ää]+/i,'ae')
    callname.gsub!(/[Üü]+/i,'ue')
    callname.gsub!(/[Öö]+/i,'oe')
    callname.gsub!(/[ß]+/i,'ss')
    callname.downcase!
    callname.gsub!(/[^a-z0-9]+/i, '-')
    callname.gsub!(/(^[-]+|[-]+$)/, '')
    return callname
  end


  def self.pretty_escape_url(str)
    return pretty_escape(create_callname(str))
  end

  def self.get_id_from_url(parameter)

    # ignore the beginning of the parameter, take last -<word>

    begin
      last_index = parameter.rindex("-")

      if last_index
        id = parameter[last_index + 1, 100]
        return id.to_i
      else
        return parameter
      end
    rescue
      return parameter
    end
  end

  def self.check_url(url)
    if url.index("http:")
      return url
    end

    return "http://#{url}"
  end

  def self.marketplace_filter_name_id(lang_id, filter_by)
    if filter_by == "all"
      return filter_by
    else

      if ! filter_by || filter_by == ""
        filter_by = "all"
      end

      begin
        return "#{self.pretty_escape_url(Category.find(filter_by).localized_categories.find_by_language_id(lang_id).name)}-#{filter_by}"
      rescue
        return "all"
      end
    end
  end

	def self.host(url)
		begin
			return URI.parse(url).host	
		rescue Exception => e
		end
		
		return ""
	end

end
