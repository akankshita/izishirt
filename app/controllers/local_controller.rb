class LocalController < FilterDisplayController
  def index
    izishirt_2011
    render :action => 'izishirt_2011'
  end
  require 'RMagick'
...
#			end
  ...
  def sitemap
    ...
    s = "/sitemaps/sitemap_#{@FULL_DOMAIN_NAME}.xml"

    redirect_to :controller => "#{s}", :lang => nil
    return
  end
  ...
  def izishirt_2011
	...
	@city_url1=params[:city_url]
	@langu=@canonical_begin_url
	
	checkcity = City.find_by_sql("select * from cities where url like '"+params[:city_url].to_s+"'")
	if checkcity.count > 0
	@city     = City.find_by_url(params[:city_url])
	@all_design_cities=Image.find(:all, :conditions=>["active=1 and city_id=?",@city.id.to_i], :order=>"rand()")
	else 
	@city     = "no"
	end
