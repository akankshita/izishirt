...
#    @coupon    = Coupon.find_by_active_and_code(true, final_code)
  ...
  def payment
	...
	@payment_total = @totaxe + @payment_subtotal.to_d	+ (@payment_shipping - @cart.shipping_rebate(@payment_shipping))
    ...
    if ENV['RAILS_ENV'] == "development"
      @paypal_email = "ldenee@karudev.com"
    end
...
#    validate_coupon(@cart.code)
  ...
  def confirm_order(order_id, type, transaction, confirm=true)
	...
	@payment_total = @totaxe + @cart.total.to_s.to_d	+ (@cart.shipping - @cart.shipping_rebate(@cart.shipping))
	
    @total    = @payment_total    #@cart.grandtotal #*rate
    @currency = session[:currency] || order.curency_id
    ...
    order.total_taxes    = @totaxe    #@cart.taxes+@cart.country_taxes
  ...
  def create_order
    ...
    order.total_price         = @cart.grandtotal.nil? ? 0 : @payment_total       #@cart.grandtotal
    order.total_shipping      = p_ship - @cart.shipping_rebate(p_ship)
    order.total_taxes         = @totaxe    #@cart.taxes+@cart.country_taxes
    order.total_rebate        = @cart.total_rebate
    order.status              = 0
...
#      @back                = get_lang_url
  ...
  def prepare_payment
    ...
    @payment_total = @totaxe + @payment_subtotal.to_f + (@payment_shipping.to_f - @cart.shipping_rebate(@payment_shipping.to_f))
